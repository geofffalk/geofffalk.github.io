...
// Utility to build a mailto link safely
...
const ProductMock = () => (
  <svg viewBox="0 0 800 480" role="img" aria-label="Aurora AR device mockup" className="w-full h-auto">
    ...
    <text x="50%" y="440" textAnchor="middle" fill="#e5e7eb" fontFamily="ui-sans-serif, system-ui" fontSize="20">Compact mixed‑reality optics • sub‑10ms pipeline • all‑day comfort</text>
  </svg>
);
...
"); // let URLSearchParams encode newlines
      ...
      <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-black/50 bg-black/30 border-b border-white/10">
        <Section className="flex items-center justify-between py-4">
          <div className="flex items-center gap-3">
            ...
            <Badge className="ml-2 bg-white/10 text-white border-white/10">alpha</Badge>
          </div>
      ...
      <Section className="pt-12 pb-20">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button className="rounded-2xl px-6 py-6 text-base" asChild>
                <a href="#contact">Request early access <ChevronRight className="ml-1 h-4 w-4"/></a>
              </Button>
              <Button variant="secondary" className="rounded-2xl px-6 py-6 text-base bg-white/10 hover:bg-white/20 border-white/10" asChild>
                <a href="#demo"><Play className="mr-2 h-4 w-4"/> Watch teaser</a>
              </Button>
            </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.05 }} className="relative">
            ...
            <Card className="relative rounded-[2rem] overflow-hidden bg-white/5 border-white/10 shadow-2xl">
          ...
          {[
            ...
            <motion.div key={f.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
              <Card className="h-full bg-white/5 border-white/10 rounded-2xl">
                <CardHeader>
                  <Badge variant="secondary" className="w-fit bg-white/10 border-white/10 text-white">{f.kpi}</Badge>
                  <CardTitle className="mt-2 text-xl">{f.title}</CardTitle>
                </CardHeader>
                <CardContent className="text-white/70">{f.text}</CardContent>
              </Card>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          ...
          <div>
            <div className="aspect-video rounded-2xl overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center">
              {/* Replace the container below with your iframe / video tag */}
        ...
        <div className="grid md:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <div className="mt-8 flex gap-4">
              <a aria-label="Twitter/X" href={CONFIG.social.twitter} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-white/80 hover:text-white">
                <Twitter className="h-5 w-5"/> Twitter/X
              </a>
              <a aria-label="Instagram" href={CONFIG.social.instagram} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-white/80 hover:text-white">
                <Instagram className="h-5 w-5"/> Instagram
              </a>
              <a aria-label="LinkedIn" href={CONFIG.social.linkedin} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-white/80 hover:text-white">
                <Linkedin className="h-5 w-5"/> LinkedIn
              </a>
              <a aria-label="YouTube" href={CONFIG.social.youtube} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-white/80 hover:text-white">
            ...
            <CardContent>
              ...
              >
                <div>
                  ...
                  <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ada Lovelace" className={`mt-1 ${CONFIG.theme.ring} rounded-xl bg-white/5 border-white/10 text-white placeholder:text-white/40`}/>
                </div>
                <div>
                  <label htmlFor="email" className="block text-sm text-white/70">Email</label>
                  <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className={`mt-1 ${CONFIG.theme.ring} rounded-xl bg-white/5 border-white/10 text-white placeholder:text-white/40`}/>
                </div>
                <div>
                  <label htmlFor="message" className="block text-sm text-white/70">Message</label>
                  <Textarea id="message" value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Tell us about your use case…" rows={5} className={`mt-1 ${CONFIG.theme.ring} rounded-xl bg-white/5 border-white/10 text-white placeholder:text-white/40`}/>
                </div>
                <div className="pt-2">
                  <Button type="submit" className="rounded-xl w-full">Send via email</Button>
                </div>
...
Email: ${email || "(your email)"}`.replace("\n", "
")
                  });
                }}
                className="flex gap-2"
              >
                <Input placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className={`rounded-xl bg-white/5 border-white/10 text-white placeholder:text-white/40 ${CONFIG.theme.ring}`}/>
      ...
      <div className="sr-only">
        {`
        Deployment tips: This component can be dropped into Next.js / Vite. Add Tailwind & shadcn/ui. Replace placeholders. For SEO add <Head> with <title>, meta, and OG tags. Ensure privacy page before launch.
        `}
      </div>
